...
from proxy.views import StipeEventRegistrationWebhook
...
class StripeEventRegistrationWebhookTestCase(TestCase):
    def setUp(self):
    ...
    def test_stripe_event_registration_webhook(self, mock_construct_event):
        ...
        response = StipeEventRegistrationWebhook.as_view()(request)

        # Assert that the response status is 200 OK
        self.assertEqual(response.status_code, 200)

        # Verify that stripe.Webhook.construct_event was called with the expected parameters
        # mock_construct_event.assert_called_once_with(
        #     b'{"key": "value"}', "test_signature", "your_test_stripe_secret_here"
        # )
