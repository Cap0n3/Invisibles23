...
def validate_names(value):
    ...
    if value.isdigit():
        ...
        raise ValidationError("Le nom ne peut pas contenir les caractères suivants : {}".format(', '.join(special_characters)))
...
def validate_address(value):
    ...
    if value.isdigit():
        ...
        raise ValidationError("L'adresse doit contenir entre 5 et 50 caractères")
         ...
         # Get the special characters in the string
        special_characters = [char for char in match if len(match) > 0]
        raise ValidationError("L'adresse ne peut pas contenir les caractères suivants : {}".format(', '.join(special_characters)))